//DESCRIPTION:Zeigt einen Dialog mit Type 1 Schriften nach dem öffnen an
//Contact: Gregor Fellenz - http://www.publishingx.de
//Version 1.0 2021-02-03


//@targetengine handleType1Warning
...
app.addEventListener("afterOpen", warnType1);

function warnType1(evt) {
    if (evt.eventType != "afterOpen") {
    ...
    if ("fonts" in evt.target) {
        ...
        for (var i = 0; i < dok.fonts.length; i++) {
            ...
            try {
                if (font.fontType == FontTypes.TYPE_1) {
                    type1Array.push(font.fullName);
                    log.warn(font.fullName);
                }
            }
            catch (e) {
                log.warn(e);
...
function initLog() {
    var version = "-1";
    var projectName = "warnType1Fonts";
    var appendLog = true;
    var debug = false;
    ...
    if (appendLog) {
        var logFile = File(logFolder + "/" + projectName + "_log.txt");
    }
    else {
        var date = new Date();
        ...
        var logFile = File(logFolder + "/" + date + "_" + projectName + "_log.txt");
    }
    if (debug) {
